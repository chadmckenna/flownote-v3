class Note < ApplicationRecord
  ...
  # Titles are file names: unique within their folder, as on a filesystem, and
  # case-insensitively so — a [[test/CLAUDE]] link matches without regard to case
  # and has to have one note to mean. The database index is the real guarantee,
  # since two simultaneous saves get past the validation; notes.title collates
  # NOCASE so the index folds exactly the way every lookup does.
  validates :title, presence: true,
                    uniqueness: { scope: :folder_id, case_sensitive: false, message: "already exists in this folder" }
